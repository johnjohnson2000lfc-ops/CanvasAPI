...
4.  Set your browser's user data directory path:
    -   Find the `USER_DATA_DIR` variable and set it to the path for your
        browser's user data. Common paths are provided as examples.
"""
...
#    If the default path for your OS is correct, you don't need to change this.
#    If you use a different browser (like Brave, Edge, or Comet), update the path.
#
#    Common paths:
#    - Windows Chrome:  r"C:\Users\YourUsername\AppData\Local\Google\Chrome\User Data"
#    - macOS Chrome:    "/Users/YourUsername/Library/Application Support/Google/Chrome"
#    - Linux Chrome:    "/home/yourusername/.config/google-chrome"
#    - Windows Edge:    r"C:\Users\YourUsername\AppData\Local\Microsoft\Edge\User Data"
USER_DATA_DIR = ""  # If empty, the script will try to auto-detect Chrome's path.
...
def get_default_user_data_dir() -> str:
    """Returns the platform-specific default path to Chrome's user data directory."""
    system = platform.system()
    if system == "Windows":
        return os.path.expanduser(r"~\AppData\Local\Google\Chrome\User Data")
    elif system == "Darwin":  # macOS
        return os.path.expanduser(r"~/Library/Application Support/Google/Chrome")
    elif system == "Linux":
        return os.path.expanduser(r"~/.config/google-chrome")
    raise NotImplementedError(f"Unsupported operating system for auto-detection: {system}")
...
def main():
    ...
    if not user_data_dir:
        try:
            ...
            print(f"Error: {e}")
            print("Please set the USER_DATA_DIR path manually in the script.")
            return

    with sync_playwright() as p:
        try:
            browser_ctx = p.chromium.launch_persistent_context(user_data_dir, headless=False)
        except Exception as e:
            print(f"Error launching browser: {e}")
            print("\nTroubleshooting:")
            print(f"1. Make sure your browser is installed and the path '{user_data_dir}' is correct.")
            print("2. Close all other running instances of the browser.")
            return
