...
ALLOWED_EXT = {
    ...
    ".rar", ".7z", ".mp3", ".mp4", ".m4a", ".webm", ".mkv", ".csv", ".json",
}
...
def get_links_from_page(page: Page) -> set[str]:
    """Finds all links on a page, including inside iframes."""
    links = set()
    ...
    # Get links from within iframes
    for frame in page.frames:
        try:
            for link in frame.locator("a[href]").all():
                href = link.get_attribute("href")
                if href:
                    links.add(urljoin(frame.url, href))
        except Exception:
            pass  # Frame might be inaccessible
